...
describe('subagent config section', () => {
  ...
  it('keeps the pool inert while the secondary-model experiment is off', async () => {
    ...
    // Flag off: the pool (and force) are ignored — omitted spawns inherit the
    // caller, but a free-form alias still binds.
    expect(resolveSubagentBinding(config, secondaryModelFlags(false), own)).toEqual({
      model: 'provider/main',
      thinking: 'medium',
    });
    expect(resolveSubagentBinding(config, secondaryModelFlags(false), own, 'provider/fast')).toEqual({
      model: 'provider/fast',
      thinking: undefined,
    });
  ...
  it('treats a pool-less default_model as an implicit single-entry pool', async () => {
    ...
    expect(resolveSubagentBinding(config, secondaryModelFlags(), own, 'provider/smart')).toEqual({
      model: 'provider/smart',
      thinking: undefined,
    });
  ...
  it('binds an alias outside the pool as a free-form [models] choice', async () => {
    const own = { modelAlias: 'provider/main', thinkingLevel: 'medium' };
    const { config, disposables } = await createConfig(
      {},
      '[secondary_model]\ndefault_model = "provider/fast"\n\n[secondary_model.models]\n"provider/fast" = "fast and cheap"\n"provider/smart" = "hard tasks"\n',
    ...
    expect(resolveSubagentBinding(config, secondaryModelFlags(), own, 'provider/typo')).toEqual({
      model: 'provider/typo',
      thinking: undefined,
    });
  ...
  it('binds a stray model choice when no pool is configured', async () => {
    const own = { modelAlias: 'provider/main', thinkingLevel: 'medium' };
    const { config, disposables } = await createConfig({});

    expect(resolveSubagentBinding(config, secondaryModelFlags(), own, 'provider/fast')).toEqual({
      model: 'provider/fast',
      thinking: undefined,
    });
