...
describe('Agent tool description', () => {
  ...
  it('still lists primary when no [secondary_model.models] pool is configured', () => {
    ctx = createTestAgent();

    const description = agentDescription();
    expect(description).toContain('Available models (pass via model):');
    expect(description).toContain('- primary: mock-model — the main model you are running on');
  });

  it('renders primary/secondary shortcuts ahead of configured aliases', () => {
    ...
    const secondaryIndex = description.indexOf(
      '- secondary: provider/fast (default) — the configured secondary / pool default model; prefer it for routine subagent tasks: fast and cheap',
    );
    ...
    expect(defaultIndex).toBeGreaterThan(secondaryIndex);
  ...
  it('marks the caller-as-default alias with both [default] and [main model]', () => {
    ...
    expect(description).toContain(
      '- secondary: mock-model (default) — the configured secondary / pool default model',
    );
  });
  ...
  it('advertises the model parameter even when no pool is configured', () => {
    ctx = createTestAgent();
  ...
  it('still advertises the model parameter while the experiment is off', () => {
    ...
    const properties = agentParameters()['properties'] as Record<string, unknown>;
    expect(properties).toHaveProperty('model');
    expect(agentDescription()).toContain('Available models (pass via model):');
  ...
  it('treats a pool-less default_model as an implicit single-entry pool', () => {
    ...
    expect(description).toContain('- secondary: provider/fast (default)');
  });
...
describe('Agent tool execution contract', () => {
  ...
  it('rejects an unconfigured free-form model alias at catalog resolution', async () => {
    const lifecycle = createAgentLifecycleStub({ createAgentIds: ['agent-child'] });
    const context = createAgentToolContext(lifecycle, secondaryModelFlags(), {
      initialConfig: {
        secondaryModel: {
    ...
    expect(result.isError).toBe(true);
    expect(result.output).toContain('provider/typo');
    expect(lifecycle.create).not.toHaveBeenCalled();
  });
...
describe('AgentSwarm tool description', () => {
  ...
  it('renders the configured pool with shortcuts and default markers', () => {
    ctx = createTestAgent(secondaryModelFlags(), {
      initialConfig: {
        secondaryModel: {
          defaultModel: 'provider/fast',
