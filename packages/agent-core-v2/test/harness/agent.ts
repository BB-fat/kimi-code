...
export class AgentTestContext {
  ...
  constructor(overrides: readonly TestAgentServiceOverride[] = [], options: TestAgentOptions = {}) {
    ...
    this.initializeAgentScope(this.agent, workspace);
  ...
  private createLifecycleStub(): IAgentLifecycleService & {
    ...
    return {
      ...
      create: (opts = {}) => {
        ...
        this.initializeAgentScope(scope, this.session.accessor.get(ISessionWorkspaceContext));
  ...
  private agentScopeSeedGroup(agentId: string): TestAgentServiceGroup {
    return (reg) => {
      ...
      const agentScope = bootstrapService.agentScope(
   ...
   * subagents: configure the profile's cwd hooks, resolve the restorable
   * services, and ignite the activity view so its constructor subscriptions
   * publish `agent.activity.updated` — production ignites it in
   * agentLifecycle.
   */
  private initializeAgentScope(scope: Scope, workspace: ISessionWorkspaceContext): void {
    scope.accessor.get(IAgentProfileService).configure({
      cwd: () => this.cwd,
  ...
  private initializeRestorableServices(scope: Scope): void {
    ...
    scope.accessor.get(IAgentBuiltinToolsRegistrar);
