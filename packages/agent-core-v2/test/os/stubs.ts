 ...
 *
 * `realCrossProcessLock()` constructs the real implementation (the pure-JS
 * protocol) for suites that exercise actual lock files in a temporary
 * directory.
 */
...
export function realCrossProcessLock(
  ...
  return new PureJsLockService(deps);
}
