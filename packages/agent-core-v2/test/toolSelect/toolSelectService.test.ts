...
import type { ToolWillExecuteContext } from '#/agent/tool/toolHooks';
...
function willContext(call: ToolCall, tool?: ExecutableTool): ToolWillExecuteContext {
  return {
    turnId: 1,
    signal: new AbortController().signal,
    toolCall: call,
    ...
    tool,
    args: {},
    execution: { approvalRule: call.name, execute: async () => ({ output: '' }) },
  };
}
