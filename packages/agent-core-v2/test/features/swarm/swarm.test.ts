...
function stubModels(ids: readonly string[] = []): IModelService {
  return {
    list: () => Object.fromEntries(ids.map((id) => [id, { model: id }])),
...
describe('AgentSwarmTool', () => {
  ...
  it('advertises the configured pool in the description only when configured', async () => {
    ...
    expect(configured.description).toContain(
      '- secondary: provider/fast (default) — the configured secondary / pool default model',
    );
    ...
    expect(unconfigured.description).toContain('Available models (pass via model):');
    expect(unconfigured.description).toContain(
      '- primary: main-model — the main model you are running on; use it for hard, quality-sensitive subagent tasks',
    );
