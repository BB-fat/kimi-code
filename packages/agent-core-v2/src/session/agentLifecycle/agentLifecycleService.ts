 ...
 * agent task manager's graceful exit policy, then drains the agent's activity
 * kernel and flushes pending wire-log appends through `wire` before releasing
 * the child scope, so the last records of a turn survive a process exit that
 * follows disposal. Bound at Session scope.
 *
