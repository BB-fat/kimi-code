 ...
 * skills win name collisions). Watches the resolved roots through `pathWatch`
 * and re-emits both filesystem changes and `plugin.onDidReload` through
 * `onDidChange`; install / enable / remove mutations deliberately do not
 * refresh the catalog — those take effect on the next explicit reload. Bound
