...
export class AgentProfileService implements IAgentProfileService {
  ...
  private resolveConfigPayload(
    ...
    // at session build. Without this, a plain `setModel` leaves the resolved
    // effort at the wire default 'off', which some models (e.g. kimi-k2.7)
    // reject with "only type=enabled is allowed".
