/**
 * `workspaceLocalConfig` domain (L2) — `IWorkspaceLocalConfigService` implementation.
 *
 * Owns the project-local `.kimi-code/local.toml` atomic-document access
 * pattern: discovers project roots, parses and validates TOML, resolves
 * additional directories with v1-compatible OS-home expansion through
 * `bootstrap`, and reads/writes the document through `hostFs`. Bound at App
 * scope.
 */
...
export class WorkspaceLocalConfigService implements IWorkspaceLocalConfigService {
