 ...
 * fallback. The advertised `model` parameter lists every configured
 * `[models]` alias plus the pool default via `buildSubagentModelDescriptions`.
 * The pool default is gated behind the `secondary-model` experiment; free-form
 * aliases stay available while it is off. Under `[secondary_model].force` the
 * parameter is not advertised at all. Swarm mode is
