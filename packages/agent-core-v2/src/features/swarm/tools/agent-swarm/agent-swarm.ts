...
export const AgentSwarmToolInputSchema = z
  .object({
    ...
    model: z
      ...
      .optional()
      .describe(
        'Model for the item-spawned subagents. Pass a configured model alias from the Available models list, or the shortcuts "primary" / "secondary". This explicit choice overrides the selected agent type\'s model_preference; without either, secondary is the default when a pool is configured, otherwise subagents inherit your model. Resumed subagents always keep their own model.',
      ),
  })
  .strict();
