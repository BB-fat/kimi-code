...
import coworkWorkerYaml from './default/cowork-worker.yaml?raw';
...
const PROFILE_SOURCES: Record<string, string> = {
  ...
  'profile/default/cowork-worker.yaml': coworkWorkerYaml,
...
export const DEFAULT_AGENT_PROFILES = loadAgentProfilesFromSources(
  ['agent.yaml', 'coder.yaml', 'cowork-worker.yaml', 'explore.yaml', 'plan.yaml'].map(
