...
3. **Spawn** — one `TowerSpawn` per mission (`kind: "worker"`, background, code-built briefing). The tool refuses duplicate names — resume the existing agent with the `Agent` tool instead. Workers commit on their branch; their completion wakes you. Spawned agents bind the configured secondary model when the secondary-model experiment is on (they inherit your model otherwise) — the resolved model is shown in the spawn output and the `spawn` line of `activity.log`.
