...
> `@` references and slash commands are two separate mechanisms: `@` gives the agent file context, while `/` invokes built-in features or Skills. A leading `/` on the first line opens the full command menu. After whitespace or on a later line, `/` offers mid-prompt built-in commands (run on select) and Skill completions.
