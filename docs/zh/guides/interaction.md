...
补全在编写提示词的过程中同样可用：在已有文本之后（单词边界处）输入 `/`，候选包含 Skill 和 plugin 命令（提交时激活，周围文本作为其输入），以及 `/plan`、`/model` 等快捷内置命令（选中时立即执行）。详见[提示词中的斜杠命令](../reference/slash-commands.md#提示词中的斜杠命令)。
...
> `@` 引用和斜杠命令是两套不同的机制：`@` 向 Agent 提供文件上下文，`/` 调用内置功能或 Skill。第一行开头的 `/` 打开完整命令菜单。空白字符后或后续行的 `/` 会提供可立即执行的 mid-prompt 内置命令，以及 Skill 补全。
...
在 TUI 中，批准计划仍可通过 `Enter` 或数字键一步完成。若要在批准时附带一段 prompt，请选中可批准的选项并按 `Tab`，输入指令后按 `Enter`；也可以直接输入非数字字符开始编辑。编辑期间按 `Esc` 会丢弃 prompt 并返回选项列表。
